...
func main() {
	...
	mux.HandleFunc("GET /api/chirps/{chirpID}", apiConfiguration.handlerGetChirpWithID)
	mux.HandleFunc("GET /api/chirps", apiConfiguration.handlerGetChirps)
	mux.HandleFunc("GET /admin/metrics", apiConfiguration.metricsHandler)
	mux.HandleFunc("PUT /api/users", apiConfiguration.handlerUpdatePassword)
	...
	mux.HandleFunc("POST /api/polka/webhooks", apiConfiguration.handlerPolkaWebhook)
